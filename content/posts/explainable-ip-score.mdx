...
![「每一分的来源」：四条判据各一行，横向条形图按分值长短排列，右侧标 −25 / −6 / +5 / +4](/figures/explainable-ip-score-1.png)

*左侧扣分、右侧加分，共用同一比例尺*

<Note type="warn">
**注意符号是反的。** 接口返回的是「风险点」——`points: 25, 6, -5, -4`，越大越差；页面呈现的是「从 100 分起扣」——`−25, −6, +5, +4`，越大越好。

两者算出来是同一个 78，但如果你拿接口的数字去对页面上的数字，会以为哪里错了。**这类「引擎值」与「展示值」的差异，在任何有渲染层的系统里都值得单独标注一次。**
</Note>
...
![四个场景卡片：AI 服务 58「勉强可用」、流媒体 77「适合」、跨境电商 67「勉强可用」、邮件发送 91「非常适合」](/figures/explainable-ip-score-2.png)
