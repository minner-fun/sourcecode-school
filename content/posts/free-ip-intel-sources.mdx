...
![报告页的「数据源与响应」：九个免费源逐个列出耗时，两个商业源标着「未启用」和缺 key 的原因](/figures/free-ip-intel-sources-1.png)

*同一份数据在报告页上的样子。两个商业源没配 key，页面如实标「未启用」并写明原因，而不是把它们藏起来*
...
![公开黑名单检索：八个 DNSBL 逐条列出 zone 域名与结果，全部「未检出」](/figures/free-ip-intel-sources-2.png)

*八个名单逐条列出，连查询用的 zone 域名都摊开——这样你可以自己 `dig` 一遍核对*
