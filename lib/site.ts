...
export const site = {
  ...
  /** 首页 <meta description> 与 WebSite 结构化数据共用；控制在 150–160 字符，Bing 把更短的判为「过短」。 */
  description:
    '接口协议还原、加密参数分析、开源项目源码拆解，以及后端、数据管道与运维在长期运行中踩过的坑。每篇都给到能复现的最小代码和实跑结果，判据来自 tsc、eslint、抓包与日志这类工具输出，而不是感觉；没验证过的部分会写明只是思路，不当成结论。面向做数据采集、逆向分析与数据工程的人，只写跑通过的过程。',
  author: {
    name: 'Minner',
    /** 关于页与文章末尾作者卡使用 */
