...
**不做 ICP 备案**（2026-09-06 定）：站点常驻 Vercel，搜索走 Google + Bing/Yandex，百度已放弃。所以站内可以有商业内容，不受个人备案那条限制。
...
### 迁到国内节点（2026-09-06 放弃）

**这条路不走了。** 备案的两个收益——用国内节点、拿百度权重——现在都不要了：
搜索走 Google + Bing/Yandex，百度对未备案站压权重本来就低到不值得，
而 Vercel 的 ~300ms 对一个内容站是可以接受的。放弃备案换来的是站内不受
「个人主体不得含经营性内容」的限制。

下面的调研留档，将来如果改主意可以直接接着用。

原目标是备案后搬到上海的阿里云节点（大陆同城延迟个位数毫秒，比 Vercel 的 ~300ms 快一个量级）。
